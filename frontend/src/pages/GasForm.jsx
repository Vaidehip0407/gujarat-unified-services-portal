...
import { ArrowLeft, RefreshCw, Flame, ExternalLink } from 'lucide-react';
...
const GasForm = () => {
  ...
  const allProviders = [
    ...gujaratData.gas.government,
    ...gujaratData.gas.private
  ];
  ...
  const fetchPrefillData = async () => {
    setLoading(true);
    try {
      const response = await api.get('/applications/prefill/gas/name_change');
      setFormData(prev => ({
        ...prev,
        consumer_number: response.data.consumer_number || '',
        bp_number: response.data.bp_number || '',
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-orange-500 to-red-500 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-white text-xl font-bold">Gas Service</h1>
                <p className="text-orange-100 text-sm">Gujarat State - Name Change Request</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-orange-50 border-b border-orange-100 p-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-orange-600">{selectedProvider.fullName}</p>
              </div>
              <a
                href={selectedProvider.portal}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-sm text-orange-700 hover:text-orange-900 bg-orange-100 px-3 py-1 rounded-lg"
              >
        ...
        <div className="p-6">
          ...
          <p className="text-gray-500 text-center text-sm mb-6">
            Please enter your details for gas connection name change
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-gray-700 mb-1">Gas Provider<span className="text-red-500">*</span></label>
              <select
                name="provider"
              ...
              >
                <optgroup label="Government">
                  {gujaratData.gas.government.map(p => (
                    <option key={p.name} value={p.name}>{p.name}</option>
                  ))}
                </optgroup>
                <optgroup label="Private">
                  {gujaratData.gas.private.map(p => (
                    <option key={p.name} value={p.name}>{p.name}</option>
                  ))}
                </optgroup>
              </select>
            ...
            <div>
              <label className="block text-gray-700 mb-1">Consumer Number / CA Number<span className="text-red-500">*</span></label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-gray-700 mb-1">BP Number</label>
              <input
                type="text"
                name="bp_number"
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  required
                />
                <button type="button" onClick={generateCaptcha} className="flex items-center gap-1 text-orange-500">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 rounded-full font-semibold hover:from-orange-600 hover:to-red-600 disabled:opacity-50"
            >
